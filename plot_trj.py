...
r_sac = np.loadtxt("dump_reward.txt")
v_sac = np.loadtxt("dump_violation.txt")
#l_sac = np.loadtxt("dump_lambda.txt")

steps = np.linspace(1,len(r_sac),len(v_sac))

pylab.figure()
pylab.plot(steps, r_sac, 'k', linewidth=3, label="safe sac with linear corr")
...
pylab.figure()
pylab.plot(steps, v_sac, 'k', linewidth=3, label="safe sac with linear corr")
